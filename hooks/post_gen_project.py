...
if not include_azure_storage or not include_azure_key_vault or not include_azure_application_insights or not include_azure_service_bus:
    rm_each([
...
for jf in workflows_dir.rglob("*.j2"):          # recurses into sub-folders
    target = jf.with_suffix("")                 # drops only the final .j2
    if target.exists():                         # safety-check: don’t overwrite
        print(f"⚠️  Skipped {jf.relative_to(ROOT)} "
